...
from config import tbcnn_ckpt_filename
from tbcnn.data_loader import DataLoader
from tbcnn.network import build_net
from tbcnn.integer.train import num_features, label_size
...
def test(proba=True):
    ...
    saver.restore(sess, tf.train.latest_checkpoint(os.path.dirname(tbcnn_ckpt_filename)))

    data_loader = DataLoader()

    pred = []
    y_test = []
    for test_nodes, test_children, test_labels in data_loader.batch_data('test', batch_size=2):
